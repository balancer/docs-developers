...
* The pool's **`creationHash`** is optional for most networks, but currently mandatory for `polygon`. (The polygonscan API does not currently support the necessary `txlistinternal` query to fetch this hash with just the `poolId`.)
...
{% hint style="info" %}
If you've generated the constructor arguments with balpy, you can continue to Step 2; you do not need to do anything on Tenderly.
{% endhint %}
